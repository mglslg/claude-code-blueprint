...
**load_session_for_agent() 与 load_session() 的区别：**

LLM 要求严格的 user/assistant 交替，而实际存储中可能有连续多条 assistant 消息（工具调用产生的多段响应），此方法将它们合并为单条。如果存在 `compressed_context`，还会在消息列表头部插入一条虚拟的 assistant 消息承载历史摘要。
